...
export const TRACKS = [
  ...
  { id: 'video',      title: 'Video & motion — clips, effects, animation' },
...
export const LESSONS = [
  ...
  {
    id: 'free-clip-effects', track: 'video', minutes: 4,
    title: 'Free clip effects — shatter glass, explode, zoom',
    summary: 'The zero-setup way to get motion: take any image you made and add an epic effect right in your browser — shatter glass, explode, zoom, glitch — then download a clip. No GPU, no account, no cost.',
    teaches: ['browser canvas effects', 'exporting a WebM clip', 'when you do NOT need an AI video model'],
    do: [{ label: 'Open the Animate tool', href: OURS.gallery }],
  },
  {
    id: 'comfyui-video', track: 'video', minutes: 15,
    title: 'Real AI video with ComfyUI (AnimateDiff / SVD)',
    summary: 'For true generative motion — a still that actually moves, camera pushes, morphs — you use a video model in ComfyUI: AnimateDiff, Stable Video Diffusion (SVD), LTX-Video or Wan. These need a GPU, so you run them on rented or your own compute (next lesson).',
    teaches: ['image-to-video (SVD)', 'text-to-video', 'AnimateDiff motion', 'why video needs a GPU'],
    do: [
      { label: 'Our ComfyUI workflow library', href: OURS.comfyui },
      { label: 'ComfyUI (open source)', href: TOOLS.comfy.url },
      { label: 'Community video workflows', href: TOOLS.github.url },
    ],
  },
  {
    id: 'run-video-yourself', track: 'video', minutes: 12,
    title: 'Run the video yourself — free & cheap GPUs',
    summary: 'Video models need a GPU. You do not have to buy one: run the ComfyUI video workflow on a free or cheap hosted GPU. Kaggle gives ~30 free GPU hours a week; Colab is ~$10/mo; Modal has free monthly credits; Fal is pay-per-clip. Same models, your compute.',
    teaches: ['free GPU (Kaggle)', 'cheap GPU (Colab/Modal/Fal)', 'nonprofit GPU credits', 'launching a video run'],
    do: [
      { label: 'Our Colab notebooks', href: OURS.colab },
      { label: 'Kaggle — 30 free GPU hrs/week', href: 'https://www.kaggle.com/code' },
      { label: `${TOOLS.colab.name}`, href: TOOLS.colab.url },
      { label: `${TOOLS.modal.name}`, href: TOOLS.modal.url },
      { label: `${TOOLS.fal.name}`, href: TOOLS.fal.url },
    ],
  },
  {
