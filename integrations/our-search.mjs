...
function defaultSources() {
  return {
    ...
    // Hathor's map of the websites (knowledge/ecosystem/hathor-site-map.md, generated by
    // tutorial/site-map.mjs from the tool registry, the Studio nav, the host routes and the wiki). One entry
    // per line, so a question like "where do I vectorize an image" lands on the exact tool + its how-to.
    // Public data only: the private lesson drafts are deliberately NOT a source here (this search also
    // feeds prompts that leave the box); they reach her through the local-only POST /lesson instead, and
    // through the `melek` source once the operator publishes them on chain.
    sitemap: async (q, { k = 4 } = {}) => {
      try {
        const fs = await import('node:fs');
        const text = fs.readFileSync(new URL('../knowledge/ecosystem/hathor-site-map.md', import.meta.url), 'utf8');
        const terms = [...new Set(String(q || '').toLowerCase().match(/[a-z][a-z0-9-]{2,}/g) || [])]
          .filter((t) => !['the', 'and', 'how', 'what', 'where', 'can', 'you', 'for', 'with', 'does'].includes(t));
        if (!terms.length) return [];
        const scored = text.split('\n').filter((l) => l.startsWith('- ')).map((l) => {
          const low = l.toLowerCase();
          return { l, hits: terms.filter((t) => low.includes(t)).length };
        }).filter((x) => x.hits > 0).sort((a, b) => b.hits - a.hits).slice(0, k);
        return rankNorm(scored.map(({ l }) => {
          const link = (l.match(/https?:\/\/[^\s)]+/) || [])[0] || '';
          const title = (l.match(/\*\*([^*]+)\*\*/) || [])[1] || 'site map';
          return { source: 'sitemap', title, link, snippet: l.replace(/^- /, '').replace(/\*\*/g, '').slice(0, 300) };
        }));
      } catch { return []; }
    },
