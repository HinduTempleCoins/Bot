// keccak.mjs — Keccak-256 (original Keccak padding 0x01, NOT NIST SHA3-256's 0x06).
//
// Monero / CryptoNote use the ORIGINAL Keccak-256 (cn_fast_hash): rate 1088 bits
// (136 bytes), capacity 512, output 32 bytes, domain-separation/pad byte 0x01. This is
// the variant that produces Monero's view keys and address checksums — do NOT swap in a
// SHA3-256 (0x06) implementation or every key and checksum will be wrong.
//
// Provenance: dependency-free Keccak-f[1600] using the unrolled 32-bit (hi/lo) lane
// formulation popularised by the public-domain js-sha3 (Chen, Yi-Cyuan, MIT) reference.
// Reimplemented here clean-room for the original-Keccak pad and verified against Monero
// address test vectors in the test suite. License: public domain / MIT-style.
//
// Pure ESM, no imports, no BigInt, no native crypto — identical in browser and Node.

// Round constants as parallel hi/lo 32-bit halves (little-endian lane convention used by
// the unrolled implementation below).
const RC = [
...
// Keccak-f[1600] on a 50-element Int32Array state s (25 lanes, [lo0,hi0,lo1,hi1,...]).
function keccakF(s) {
  let h, l, n, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, b0, b1, b2, b3, b4, b5, b6, b7, b8, b9;
  let b10, b11, b12, b13, b14, b15, b16, b17, b18, b19, b20, b21, b22, b23, b24, b25, b26;
  let b27, b28, b29, b30, b31, b32, b33, b34, b35, b36, b37, b38, b39, b40, b41, b42, b43;
  let b44, b45, b46, b47, b48, b49;
  for (n = 0; n < 48; n += 2) {
...
/**
 * keccak256(message) -> Uint8Array(32). Original Keccak (pad byte 0x01), rate 136 bytes.
 * @param {Uint8Array} msg
 */
export function keccak256(msg) {
  const rate = 136; // bytes; 17 lanes of 8 bytes
  const input = msg instanceof Uint8Array ? msg : new Uint8Array(msg);
  const s = new Int32Array(50);

  let offset = 0;
  const total = input.length;
  // Absorb. We process the padded message in `rate`-byte blocks.
  const blocks = Math.floor(total / rate) + 1; // +1 always, last is the padded block
  for (let bi = 0; bi < blocks; bi++) {
    const isLast = bi === blocks - 1;
    const blk = new Uint8Array(rate);
    if (!isLast) {
      blk.set(input.subarray(offset, offset + rate));
      offset += rate;
    } else {
      const rem = total - offset;
      blk.set(input.subarray(offset, total));
      blk[rem] = 0x01;
      blk[rate - 1] |= 0x80;
    }
    // XOR block into state as little-endian 32-bit words (2 words per lane).
    for (let i = 0; i < rate; i += 4) {
      const word =
        (blk[i] | (blk[i + 1] << 8) | (blk[i + 2] << 16) | (blk[i + 3] << 24)) | 0;
      s[i >> 2] ^= word;
    }
    keccakF(s);
  }

  // Squeeze 32 bytes from the first 8 words (little-endian).
