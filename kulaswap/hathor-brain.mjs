// VENDORED (browser copy) of src/trollbox/index.mjs — Hathor’s deterministic chat brain, run
// CLIENT-SIDE on the pool so her chat box is self-contained: no server, no CORS, no chain.
// Regenerate by re-copying src/trollbox/index.mjs and nulling loadMenu if that brain changes.
...
async function loadMenu() {
  if (!_menuModulePromise) {
    _menuModulePromise = Promise.resolve(null); // browser: no node menu module; !commands degrades gracefully
  }
