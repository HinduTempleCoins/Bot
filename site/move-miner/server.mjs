...
export async function handler(req, res) {
  try {
    ...
    if (path === '/robots.txt') {
      ...
      return res.end(sharedRobotsTxt((process.env.BASE_URL || 'https://move.melek.salon').replace(/\/$/, '')));
    }
    if (path === '/sitemap.xml') {
      // This host is listed in the network sitemap-index; an index entry that 404s wastes the crawl.
      const today = new Date().toISOString().slice(0, 10);
      res.writeHead(200, { 'content-type': 'application/xml; charset=utf-8' });
      return res.end(sharedSitemapXml((process.env.BASE_URL || 'https://move.melek.salon').replace(/\/$/, ''), ["/"].map((pp) => ({
