...
<header class=topbar><a class=brand href="${bp('/')}">📝 SoapBox <span>Notes</span><span class=alpha>Alpha</span></a>
  <div class=topbar-r>${TOOLS_NAV}<a href="${bp('/')}">New</a><button type=button id=nav-save>☁ Sync across devices</button></div></header>
...
export async function handler(req, res) {
  try {
    ...
    return res.end(page('Not found — SoapBox Notes', '<h1>Not found</h1><p class=muted>That page doesn\'t exist. <a href="' + bp('/') + '">Open the notepad</a>.</p>', { robots: 'noindex,follow' }));
  } catch (e) {
