// notes.test.mjs — OFFLINE tests for SoapBox Notes (drives `handler` with a mock req/res; no port
// bound, no network). Mirrors site/diagram/diagram.test.mjs. Verifies: the scratchpad renders with a
// textarea + autosave + export, /health is ok JSON, robots/sitemap/sitemap-index/llms serve, every
// localStorage access is try/catch-guarded, a hostile <script> in the echoed title param is escaped, a
// hostile return URL is neutralised, no crypto pitch up front, and an unknown path is a 404 (never 500).

import { test } from 'node:test';
import assert from 'node:assert';
import { handler, notesPage, esc, safeHref, SITEMAP_PATHS } from './server.mjs';
...
async function get(path, headers = {}) {
  const res = mockRes();
  await handler({ url: path, headers: { host: 'notes.test', ...headers } }, res);
  return res;
}

test('home 200 renders the scratchpad (textarea + autosave status + export)', async () => {
  const res = await get('/');
  assert.equal(res.code, 200);
  assert.match(res.headers['content-type'], /text\/html/);
  assert.match(res.body, /textarea id=note/);            // the note input
  assert.match(res.body, /id=status/);                   // the autosave status
  assert.match(res.body, /id=export/);                   // export .txt button
  assert.match(res.body, /Free online notepad/i);        // reads like a normal free tool
});
...
test('autosaves to localStorage, and every access is try/catch guarded', async () => {
  ...
  assert.match(body, /autosave/i);                       // and says so
  // every getItem/setItem/removeItem must sit inside a try{...}catch — assert there is no bare access
  // outside a try by requiring at least as many try blocks as write sites (belt-and-braces heuristic).
...
test('the MELEK unlock is understated and opt-in — no crypto pitch up front', async () => {
  const body = (await get('/')).body;
  assert.match(body, /sync (across devices|your notes)/i);   // the understated unlock CTA
  ...
  const first = body.indexOf('Free online notepad');
...
test('robots.txt, sitemap.xml, sitemap-index.xml, llms.txt all serve', async () => {
  ...
  assert.match(llms.body, /Notepad/i);
});
...
test('never throws on a garbage URL', async () => {
  const res = mockRes();
  await handler({ url: '/%%%bad%%', headers: { host: 'notes.test' } }, res);
  assert.ok(res.code === 404 || res.code === 500 || res.code === 200);
});

test('notesPage() is a pure string', () => {
